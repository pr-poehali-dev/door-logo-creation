...
// Логотип МДК по референсу:
// — М и Д делят общую стойку (нет зазора между буквами)
// — Д открытая снизу: ножки выступают ниже базовой линии
// — Д и К делят общую стойку справа
// — V-вырез М неглубокий (~45% высоты), лучи К широкие и жирные
// Сетка 210×90 (буквы y0–75, ножки Д y75–90)

const MDKLogo = ({ col = "#F5EFE0", size = 1 }: { col?: string; size?: number }) => (
  <svg width={210 * size} height={90 * size} viewBox="0 0 210 90" fill={col} xmlns="http://www.w3.org/2000/svg">

    {/* ══ М ══  x: 0–88, y: 0–75
        Левая стойка: x0–20
        V-вырез вершина: x=44, y=38 (неглубокий, ~50% высоты)
        Правая стойка М совмещена с левой стойкой Д: x68–88
        Рисуем М как единый силуэт с вырезом через clipPath */}

    {/* Левая стойка */}
    <rect x="0" y="0" width="20" height="75"/>
    {/* Левый скат V (заполнение между стойкой и вершиной) */}
    <polygon points="20,0 44,0 44,9 32,38 20,38"/>
    {/* Правый скат V */}
    <polygon points="44,0 68,0 68,38 56,38 44,9"/>
    {/* Правая стойка М / левая Д */}
    <rect x="68" y="0" width="20" height="75"/>

    {/* ══ Д ══  x: 68–142, y: 0–90
        Верхняя перемычка: y0–18
        Тело: пустое внутри (арка)
        Нижняя полка: y57–75
        Ножки ниже: y75–90
        Правая стойка = левая К: x122–142 */}

    {/* Верхняя перемычка (только средняя часть, стойки уже есть) */}
    <rect x="88" y="0" width="34" height="18"/>
    {/* Нижняя полка Д (средняя часть) */}
    <rect x="82" y="57" width="52" height="18"/>
    {/* Левая ножка Д */}
    <rect x="68" y="57" width="18" height="33"/>
    {/* Правая стойка Д / левая К */}
    <rect x="122" y="0" width="20" height="75"/>
    {/* Правая ножка Д */}
    <rect x="124" y="57" width="18" height="33"/>

    {/* ══ К ══  x: 122–210, y: 0–75
        Стойка К = правая Д (x122–142) уже нарисована.
        Лучи расходятся из точки пересечения (~x158, y37) — как на фото.
        Верхний луч: трапеция от стойки вверх-вправо
        Нижний луч: трапеция от стойки вниз-вправо
        Зазор между лучами образует острый треугольник, острым углом влево */}

    {/* Верхний луч К */}
    <polygon points="142,0 210,0 158,37 142,37"/>
    {/* Нижний луч К */}
    <polygon points="142,38 158,38 210,75 142,75"/>
...
const LogoFull = ({ variant = "dark", size = 1, monogram = 1 }: { variant?: "dark" | "light" | "mono-dark" | "mono-light"; size?: number; monogram?: number }) => {
  ...
  return (
    <div style={{ background: bg, padding: `${20 * size}px ${28 * size}px`, display: "inline-flex", alignItems: "center", gap: `${20 * size}px`, borderRadius: `${4 * size}px` }}>
      <MDKLogo col={colVal} size={size * 0.42} />
