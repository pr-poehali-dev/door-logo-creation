...
// ─── ЛОГОТИП МДК — точное воспроизведение референса ─────────────────────────
// Сетка 300×100. Все три буквы одинаковой высоты (0–100).
// М и Д делят стойку x88–112. Д и К делят стойку x188–212.
// Д: ножки внутри высоты (не выступают), П-образная открытая снизу.
// К: лучи расходятся из середины (y=50), зазор между ними ~10px у стойки.

const MDKLogo = ({ col = "#F5EFE0", size = 1 }: { col?: string; size?: number }) => {
  // Толщина всех штрихов
  const S = 22;
  // Высота
  const H = 100;
  // Координаты стоек (левые края)
  // М: левая стойка 0, правая/общая с Д: 88
  // Д: левая/общая с М: 88, правая/общая с К: 188
  // К: левая/общая с Д: 188, правый край: 300
  // Ширина М = 88+S=110 (правая стойка до 110), Д = 188+S-88=112, К = 300-188=112

  // М как единый path с вырезом V:
  // Силуэт снаружи: 0,0 → 110,0 → 110,H → 0,H
  // Вырез V внутри: от (S,0) вниз к вершине (55, 48) обратно к (88,0)
  // Рисуем через evenodd fill-rule

  return (
  <svg width={300 * size} height={H * size} viewBox={`0 0 300 ${H}`} xmlns="http://www.w3.org/2000/svg">

    {/* М — с V-вырезом, evenodd */}
    <path
      fill={col}
      fillRule="evenodd"
      d={`
        M 0,0 L 110,0 L 110,${H} L 0,${H} Z
        M ${S},0 L 55,48 L 88,0 Z
      `}
    />

    {/* Д — П-образная открытая снизу
        Силуэт: x88–212, y0–H
        Вырез внутри: x110–190, y0–(H-S) → нижний край без ножек
        Ножки: оставляем x88–(88+S) и x(212-S)–212 до самого низа
    */}
    <path
      fill={col}
      fillRule="evenodd"
      d={`
        M 88,0 L 212,0 L 212,${H} L 88,${H} Z
        M 110,0 L 190,0 L 190,${H - S} L 110,${H - S} Z
      `}
    />

    {/* К — стойка + два луча расходятся из середины
        Стойка: x188–212, y0–H (общая с Д, уже в Д — но нарисуем отдельно чисто)
        Верхний луч: трапеция (212,0)→(300,0)→(248,50)→(212,43)
        Нижний луч: трапеция (212,57)→(248,50)→(300,H)→(212,H)
        Зазор между лучами ~14px у стойки (y43–y57)
    */}
    <rect x="188" y="0" width={S} height={H} fill={col}/>
    {/* Верхний луч */}
    <polygon points={`212,0 300,0 252,43 212,43`} fill={col}/>
    {/* Нижний луч */}
    <polygon points={`212,57 252,57 300,${H} 212,${H}`} fill={col}/>
